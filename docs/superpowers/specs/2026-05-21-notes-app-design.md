...
| 阶段 | 范围 | 交付物 |
|------|------|--------|
| P0 | Monorepo 搭建、构建工具链、开发环境 | 可运行的空项目骨架 |
| P1 | 数据模型、StorageAdapter、SQLite + IndexedDB、CRUD API、Zustand store | 笔记 CRUD 通过 API 可用 |
| P2 | TipTap 编辑器集成、Markdown 快捷输入、模式切换、自动保存 | 文本/Markdown 笔记可编辑并持久化 |
| P3 | 图片/视频自定义扩展、上传流程、缩略图、响应式渲染 | 富媒体笔记可用 |
| P4 | 桌面双栏布局、移动端堆栈导航、文件夹树、标签、搜索、暗色/亮色主题 | 完整 UI + 响应式布局 |
| P5 | PWA、离线支持、数据导出/导入（Markdown + JSON）、回收站 | 生产可用 Web 应用 |
| P6 | Yjs 同步引擎、同步服务端、公开分享链接 | 多设备同步 + 分享 |