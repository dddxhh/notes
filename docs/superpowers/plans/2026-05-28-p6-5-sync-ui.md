...
**Architecture:** 新增 authStore 管理用户认证状态（JWT token 存 sessionStorage）。LoginPanel/RegisterPanel 为共享组件，在 Sidebar 设置面板和 MobileSettings 中内嵌显示（非独立页面）。SyncStatusIndicator 显示同步状态图标。SyncSettingsPanel 包含服务端地址配置、登录/登出、附件同步策略。
...
git commit -m "feat: restore auth state from sessionStorage on app start"
```
