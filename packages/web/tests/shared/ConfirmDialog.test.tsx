...
vi.mock("@radix-ui/react-alert-dialog", () => ({
  Root: ({ open, onOpenChange, children }: any) =>
    open ? <div data-alert-dialog-root>{children}</div> : null,
  ...
  Action: ({ children, className, onClick, asChild }: any) =>
    asChild ? (
      children
    ) : (
  ...
  Cancel: ({ children, className, onClick, asChild }: any) =>
    asChild ? (
      children
    ) : (
      <button className={className} onClick={onClick || (() => mockOnOpenChange(false))}>
        {children}
      </button>
    ),
}));
