...
vi.mock("@radix-ui/react-dialog", () => ({
  ...
  Close: ({ children, className }: any) => <button className={className} onClick={() => mockOnOpenChange(false)}>{children}</button>,
}));
...
describe("RenameDialog", () => {
  ...
  it("renders with input when open", () => {
    render(<RenameDialog open={true} onOpenChange={mockOnOpenChange} currentName="My Note" onRename={mockOnRename} />);
    expect(screen.getByDisplayValue("My Note")).toBeTruthy();
  });

  it("does not render when closed", () => {
    render(<RenameDialog open={false} onOpenChange={mockOnOpenChange} currentName="My Note" onRename={mockOnRename} />);
    expect(screen.queryByDisplayValue("My Note")).toBeNull();
  });

  it("typing new name and clicking confirm calls onRename", async () => {
    const user = userEvent.setup();
    render(<RenameDialog open={true} onOpenChange={mockOnOpenChange} currentName="Old Name" onRename={mockOnRename} />);
  ...
  it("pressing Enter calls onRename with current input value", async () => {
    const user = userEvent.setup();
    render(<RenameDialog open={true} onOpenChange={mockOnOpenChange} currentName="Note Title" onRename={mockOnRename} />);
