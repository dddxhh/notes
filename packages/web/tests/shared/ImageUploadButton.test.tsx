...
describe("ImageUploadButton", () => {
  it("renders button with image icon and title", () => {
    const { container } = render(<ImageUploadButton onFileSelected={vi.fn()} />);
    const button = container.querySelector("button")!;
    expect(button.textContent).toContain("🖼");
    expect(button.title).toBe("插入图片");
  });
  ...
  it("uses custom className when provided", () => {
    const { container } = render(<ImageUploadButton onFileSelected={vi.fn()} className="custom-btn" />);
    const button = container.querySelector("button")!;
  ...
  it("calls onFileSelected when a file is selected", () => {
    ...
    const { container } = render(<ImageUploadButton onFileSelected={onFileSelected} />);
    const input = container.querySelector("input")!;
