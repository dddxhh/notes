...
vi.mock("../src/stores", () => ({
  ...
  useTagsStore: (selector?: any) => selector ? selector({ tags: mockTags, setTags: mockSetTags, addTag: vi.fn(), removeTag: vi.fn(), loading: false, setLoading: vi.fn() }) : { tags: mockTags },
}));
...
vi.mock("../src/components/shared/NoteCard", () => ({
  default: ({ note, onClick, tags }: any) => (
    <div data-testid="note-card" onClick={() => onClick(note)}>
      {note.title}
      {tags?.map((t: any) => <span key={t.id} data-testid="note-card-tag">#{t.name}</span>)}
    </div>
  ),
}));
...
vi.mock("../src/components/shared/SearchBar", () => ({
  default: ({ onQueryChange }: any) => (
    <button data-testid="search-entry-btn" onClick={() => onQueryChange("test")}>搜索</button>
  ),
}));
...
describe("QuickNote", () => {
  ...
  it("renders NoteCard with tag badges when tags are provided", () => {
    mockNotesState.notes = [
      { id: "n1", title: "My Note", contentJson: "", mdText: "", folderId: null, type: "rich", createdAt: Date.now(), updatedAt: Date.now(), deletedAt: null, version: 1 },
    ];
    render(<QuickNote />);
