...
export default function SearchFilterPanel({ folders, tags, filter, onFilterChange }: SearchFilterPanelProps) {
  ...
  return (
    <div className="flex flex-col gap-3 p-3 rounded-lg" style={{ backgroundColor: "var(--bg-secondary)" }}>
      <div>
        <label className="text-sm font-medium block mb-1" style={{ color: "var(--text-primary)" }}>文件夹</label>
        <select
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <span className="text-sm font-medium" style={{ color: "var(--text-primary)" }}>标签筛选</span>
          <button
        ...
        <div className="flex flex-wrap gap-2">
          {tags.map((tag) => (
            <button
              ...
              style={!filter.tagIds?.includes(tag.id) ? { backgroundColor: "var(--bg-tertiary)", color: "var(--text-primary)" } : undefined}
            >
              {tag.name}
            </button>
          ))}
      ...
      <div>
        <span className="text-sm font-medium block mb-1" style={{ color: "var(--text-primary)" }}>时间范围</span>
        <select
